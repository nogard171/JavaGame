...
public class Game extends Window
{
	...
	try
	{
	    login_bg.texture = TextureLoader.getTexture("PNG",
		    ResourceLoader.getResourceAsStream("resources/images/bg.png"));
	    test.texture = TextureLoader.getTexture("PNG",
		    ResourceLoader.getResourceAsStream("resources/images/character_set.png"));
	}
	catch (IOException e)
	{
	...
	if (game_State == State.GAME)
	{
	    ...
	    if (super.keyboard.keyPressed(Keyboard.KEY_LSHIFT))
	    {
		speed += 10;
	    }
	    ...
	    if (moved)
	    {
		int tps = (int) ((speed / 32) * 10);
		Display.setTitle("TPS:" + tps);
	    }
	...
	if (game_State == State.GAME)
	{
	    ...
	    /*
	     * if(network.map!=null&&network.map.tiles.containsKey(test.x+","+
	     * test.y))
	     * {
	...
	{
	    GL11.glDisable(GL11.GL_TEXTURE_2D);
	    if (network.map != null && network.map.chunks != null)
	    {

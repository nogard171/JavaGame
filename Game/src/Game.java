...
public class Game implements Runnable {
	...
	private void initOpenGL() {
		...
		loader.loadData("resources/models/test.obj");
		...
		for (int x = 0; x < map.width - 1; x++) {
			for (int z = 0; z < 1; z++) {
				for (int y = 0; y < map.height - 1; y++) {
					...
					obj.y = z * 2;
					obj.z = y * 2;
					obj.vectors = loader.getVectors();
					obj.faces = loader.getFaces();
					obj.texture = loader.getTexture();
					obj.textureFaces = loader.getTextureFaces();
					obj.textureVectors = loader.getTextureVectors();
